...
func New(nt *neotrellis.Device, dfp *dfplayer.Player, getter VolumeGetter) (*Player, error) {
	...
	// play previous
	err := nt.SetPixelColor(0, 0, neotrellis.RGB{0, 100, 150})
	if err != nil {
		return nil, err
	}
	...
	// play next
	err = nt.SetPixelColor(1, 0, neotrellis.RGB{0, 150, 100})
	if err != nil {
		return nil, err
	}
	...
	//stop
	err = nt.SetPixelColor(2, 0, neotrellis.RGB{0xFF, 0, 0})
	if err != nil {
		return nil, err
	}
	...
	// volume
	err = nt.SetPixelColor(3, 1, neotrellis.RGB{150, 100, 0})
	if err != nil {
		return nil, err
	}
	...
	err = nt.SetPixelColor(3, 0, neotrellis.RGB{100, 150, 0})
	if err != nil {
		return nil, err
	}
